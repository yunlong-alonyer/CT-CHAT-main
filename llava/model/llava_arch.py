...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            ...
            # 经过 CT Encoder 和 Attention-Pooler 后，输出维度通常为 [Batch, 256, 5120]
            image_features = self.encode_images(images)

            # ==========================================================
            # 修改 2：对于 3D CT 序列，强制使用 flat 模式，跳过 2D AnyRes 逻辑
            # ==========================================================
            mm_patch_merge_type = getattr(self.config, 'mm_patch_merge_type', 'flat')

            if mm_patch_merge_type == 'flat' or 'ct' in str(getattr(self.config, 'mm_projector_type', '')):
                # 如果是 flat 或者我们自定义的 ct projector，直接作为独立的 token 序列
                # 此时 image_features 变成 list，每个元素形如 [256, 5120]
                if isinstance(image_features, torch.Tensor):
                    image_features = [x for x in image_features]
            elif mm_patch_merge_type.startswith('spatial'):
                # (保留原版 2D 逻辑以防你需要兼顾其他任务，但 CT 任务不应进入此分支)
                # ... [此处保留你原代码中 spatial_unpad 相关的很长一段代码] ...
                raise NotImplementedError(
                    "3D CT 模型不应该使用 spatial merge type，请在 config 中设置 mm_patch_merge_type='flat'")
            else:
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_images == 0:
                ...
                # ==========================================================
                # 修改 4：使用更安全的 self.get_input_embeddings() 方法
                # ==========================================================
            ...
            # 使用 self.get_input_embeddings() 获取纯文本部分的 Embedding
        ...
        # LLaVA 架构的核心：返回拼装好的 input_embeds，并将 input_ids 置为 None，跳过大模型原有的 Embedding 层
