...
class CTViT(nn.Module):
    def __init__(
        ...
        self.to_pixels = nn.Sequential(
            ...
            Rearrange('b t h w (c pt p1 p2) -> b c (t pt) (h p1) (w p2)', p1=patch_height, p2=patch_width,
                      pt=temporal_patch_size),
        )
    ...
    def frames_per_num_tokens(self, num_tokens):
        ...
        assert (
                           num_tokens % tokens_per_frame) == 0, f'number of tokens must be divisible by number of tokens per frame {tokens_per_frame}'
        assert (num_tokens > 0)
    ...
    def forward(
        ...
        *_, h, w, _ = shape
        ...
        if video.shape[1] == 1:
            input_vgg_input2, recon_vgg_input2 = map(lambda t: repeat(t, 'b 1 ... -> b c ...', c=3),
